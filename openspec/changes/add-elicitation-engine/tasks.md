...
- [ ] 5.4 ~~`listTurns`~~. **Decidí no implementarla** como server action: la página RSC (`/projects/[id]/elicit/page.tsx`) hace la query directamente con el cliente Supabase y `revalidatePath` invalida cuando hay mutaciones. Una action aparte solo duplicaría código. Si en el futuro la UI necesita paginar/filtrar desde el cliente, se añade entonces.
...
- [ ] 6.3 Logging server-side estructurado de cada llamada al modelo. **Pendiente** — `callModel` solo lanza/retorna pero no escribe a `console`. Se atiende en un cambio menor o en `add-elicitation-streaming`, junto con telemetría.
...
- [ ] 8.3 Smoke test manual: crear proyecto → iniciar sesión → enviar 3 turnos → forzar respuesta vaga ("queremos algo rápido") y verificar que aparece un `detected_issue` de tipo `vagueness` → saltar de sección → cerrar sesión. **Pendiente del usuario** — requiere interacción real con la UI y consumo de tokens contra la API de Anthropic.
